  ...
  try {
    ...
    files.forEach(i => {
      ...
      let assignObj = {
        ...
        opts: item.hasOwnProperty('_opts') ? _opts : opts,
        _customOpts: item.hasOwnProperty('__customOpts') ? __customOpts : _customOpts,
        ossParams: item.hasOwnProperty('_ossParams') ? _ossParams : ossParams
      }
      // assignObj.opts.ossParams = item.hasOwnProperty('_ossParams')?_ossParams:ossParams
      ...
      assignObj._opts = JSON.parse(JSON.stringify(assignObj.opts))
      assignObj.__customOpts = JSON.parse(JSON.stringify(assignObj._customOpts))
      assignObj._ossParams = Object.assign({}, assignObj.ossParams)
      ...
      if (i.chunks && i.chunks.length) {
        if (_resumeLog && fs.existsSync(_resumeLog)) {
          let resumeLogStr = fs.readFileSync(_resumeLog).toString()
          let resumeLog = JSON.parse(resumeLogStr || '[]')
          if (resumeLog && resumeLog.length) {
            ...
            if (isQiniuCtx && i.chunks.length === 1) {
              ...
              // let cks = JSON.parse(resumeLogStr || '[]')
              // i.chunks = cks && cks.length?cks:i.chunks
              i.chunks.forEach(ic => {
